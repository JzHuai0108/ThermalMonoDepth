...
DATA_ROOT=/media/pi/BackupPlus/jhuai/data/rrxio/irs_rtvi_datasets_2021
RESULTS_DIR=/home/pi/Desktop/temp/results/
...
for NAME in ${NAMES[@]}; do
	...
	for SEQ in ${SEQS[@]}; do
		...
		# Detph Evaulation 
		cmd="CUDA_VISIBLE_DEVICES=${GPU_ID} python test_disp.py \
		--resnet-layers $RESNET --pretrained-dispnet $DISP_NET \
		--img-height $IMG_H --img-width $IMG_W \
		--dataset-dir ${DATA_DIR} --output-dir $OUTPUT_DEPTH_DIR"
		echo $cmd
		$cmd >> ${OUTPUT_DEPTH_DIR}/disp.txt
